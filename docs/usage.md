...
- `request_user_input_async`
  - 显示独立的“需要补充输入”表单，在所有 quiet 模式下均可见；后续答复不会覆盖问题
  - 可选择选项或填写其它答案，点击“提交”才会发送；填写期间任务继续运行
  - 回答会补充到同一会话；如果提问的 turn 已结束，回答会发起下一轮对话
  - 可以取消问题而不停止任务；切换会话后旧问题不能提交到新会话，服务重启后旧表单失效
