...
func TestDriveFileSharerShareAndCleanup(t *testing.T) {
	...
	if api.files[0].Name != "report.txt" {
		t.Fatalf("uploaded shared file name = %q, want report.txt", api.files[0].Name)
	}
	children := api.children["folder-1"]
	if len(children) != 1 || children[0].Type != previewFolderType {
		t.Fatalf("artifact child folders = %+v, want 1 folder", children)
	}
...
func TestFileShareHelpers(t *testing.T) {
	if got := artifactFolderName(time.Unix(1700000000, 123).UTC(), "abc"); !strings.Contains(got, "abc") {
		t.Fatalf("artifactFolderName() = %q", got)
	}
	if ts, ok := artifactFolderCreatedTime(artifactFolderName(time.Unix(1700000000, 123).UTC(), "abc")); !ok || ts.IsZero() {
		t.Fatalf("artifactFolderCreatedTime() = %v, %v", ts, ok)
	}
