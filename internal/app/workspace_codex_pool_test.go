...
func TestWorkspaceCodexPoolWebSocketUsesSharedClientAndPreservesWorkspaceCWD(t *testing.T) {
	root := t.TempDir()
	cfg := config.Default()
	cfg.Codex.Transport = "ws"
	cfg.Codex.WSURL = "ws://example.test/ws"
	cfg.Codex.AppServerDir = filepath.Join(root, "global-app")
	cfg.Workspaces = []config.Workspace{
		{ID: "ws1", Cwd: filepath.Join(root, "ws1"), AppServerDir: filepath.Join(root, "ws1-app")},
		{ID: "ws2", Cwd: filepath.Join(root, "ws2"), AppServerDir: filepath.Join(root, "ws2-app")},
	}

	var (
		created    []*fakeCodexClient
		factoryCfg []config.CodexConfig
		calls      []string
		cwds       []string
	)
	pool := newWorkspaceCodexPool(cfg, func(c config.CodexConfig) codexClient {
		fc := &fakeCodexClient{}
		created = append(created, fc)
		factoryCfg = append(factoryCfg, c)
		fc.callHook = func(_ context.Context, method string, params any, out any) error {
			calls = append(calls, method)
			cwds = append(cwds, routeCWDFromParams(params))
			if method == "thread/start" {
				result := out.(*codexrpc.ThreadStartResult)
				result.Thread.ID = fmt.Sprintf("thread-%s", filepath.Base(routeCWDFromParams(params)))
			}
			return nil
		}
		return fc
	...
	if len(created) != 1 {
		t.Fatalf("created clients after Start = %d, want shared websocket client", len(created))
	}
	if !created[0].started {
		t.Fatal("expected shared websocket client to be started")
	}
	if got := factoryCfg[0].AppServerDir; got != cfg.Codex.AppServerDir {
		t.Fatalf("factory AppServerDir = %q, want unchanged global config %q in ws mode", got, cfg.Codex.AppServerDir)
	}
	...
	if len(created) != 1 {
		t.Fatalf("created clients after thread/start calls = %d, want still 1 shared websocket client", len(created))
	}
	if len(calls) != 2 || calls[0] != "thread/start" || calls[1] != "thread/start" {
		t.Fatalf("shared websocket client calls = %+v, want two thread/start calls", calls)
	}
	if len(cwds) != 2 || cwds[0] != cfg.Workspaces[0].Cwd || cwds[1] != cfg.Workspaces[1].Cwd {
		t.Fatalf("thread/start cwds = %+v, want workspace cwds [%q %q]", cwds, cfg.Workspaces[0].Cwd, cfg.Workspaces[1].Cwd)
	}
}
