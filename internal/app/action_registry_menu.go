...
var menuCardActionHandlers = map[string]cardActionHandler{
	...
	"menu.auto_retry": func(a *App, action *feishu.CardAction) (*callback.CardActionTriggerResponse, error) {
	...
	"auto_retry.set": func(a *App, action *feishu.CardAction) (*callback.CardActionTriggerResponse, error) {
		return a.completeAutoRetrySet(action, strings.EqualFold(actionStringValue(action, "enabled"), "on"))
	},
}
