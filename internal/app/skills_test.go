...
func TestCommandSkillsRendersCardFromAppServer(t *testing.T) {
	...
	if err := a.commandSkills(msg, nil); err != nil {
...
func TestCommandSkillsReloadForcesReload(t *testing.T) {
	...
	if err := a.commandSkills(msg, []string{"reload"}); err != nil {
...
func TestCompleteSkillsSelectStoresPendingSkill(t *testing.T) {
	...
	resp, err := a.completeSkillsSelect(&feishu.CardAction{Option: wantSkill.Path}, sessionKey, wantSkill.Path)
	if err != nil {
	...
	if got, ok := a.sessionPendingSkill(sessionKey); !ok || got.Name != wantSkill.Name || got.Path != wantSkill.Path {
...
func TestCompleteSkillsSelectRejectsDisabledSkill(t *testing.T) {
	...
	resp, err := a.completeSkillsSelect(&feishu.CardAction{Option: disabled.Path}, sessionKey, disabled.Path)
	if err != nil {
	...
	if _, ok := a.sessionPendingSkill(sessionKey); ok {
...
func TestEnqueueSubmissionUsesPendingSkillWithoutListingSkills(t *testing.T) {
	...
	a.setSessionPendingSkill(sessionKey, state.SubmissionSkill{Name: "openai-docs", Path: "/skills/openai-docs"})
...
func TestEnqueueSubmissionExplicitSkillPrefixOverridesPending(t *testing.T) {
	...
	sessionKey := a.makeSessionKey(msg)
	a.setSessionPendingSkill(sessionKey, state.SubmissionSkill{Name: "old-skill", Path: "/skills/old"})
...
func TestEnqueueSubmissionSkillOnlySetsPendingSkill(t *testing.T) {
	...
	if got, ok := a.sessionPendingSkill(sessionKey); !ok || got.Name != "openai-docs" || got.Path != "/skills/openai-docs" {
...
func TestTrySteerInboundReplyIgnoresSkillSemantics(t *testing.T) {
	...
	if pending, ok := a.sessionPendingSkill(sessionKey); !ok || pending.Name != "openai-docs" {
