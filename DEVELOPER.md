...
- A control that returns to the previous menu level is always labelled with the shared `feishu.MenuBackButtonText` (`返回上一级`), referenced as a constant rather than retyped. Never label it after its destination (`返回模型配置`, `返回工作区管理`, `返回常用工具`, `返回 thread`, ...) and never append a command hint to it: the breadcrumb in the card body already says where the card sits, forward controls carry the command hints, and both back-control orderings match this exact text. A destination-named back control therefore also silently loses its "final action" position. The label is not configurable per card, per command, or per chat scope.
...
- Group workspace management has one additional scope-specific action: show `解除本群绑定` in place of the single-chat `删除工作区` action. Unbinding clears only the current Bot's workspace association with that group and returns the group to workspace onboarding; it must not remove local workspace configuration or directory contents. This prevents a group-level action from appearing to delete a machine-wide resource.
...
- Group primary is a frontend-scoped boolean for each group. Persist `GroupPrimary.Enabled` under a frontend-specific key; process `@Bot /primary on` and empty `@Bot` assignments on every frontend, setting `Enabled=true` only when the live Bot open_id matches the sole mention and `false` otherwise. Do not persist a shared owner identity or reuse another frontend's state.
...
- A `工作中` card may be reused by the first substantive card only when it contains only the reasoning placeholder (`思考中...`). Examples of substantive cards are approval requests, tool user input, elicitation, agent messages, plans, final output, and terminal status cards.
- Once a `工作中` card contains command/file/search/tool progress, it is itself substantive content and must not be patched into approval, final, or terminal output.
- If an approval or other server-request card reuses a reasoning-only `工作中` card, later final output must be sent as a new reply card instead of patching that approval card.
- If a working card already exists and the mode changes, turn-boundary and turn-finish code must still drain it using the same reasoning-only reuse rule instead of leaving it orphaned.
